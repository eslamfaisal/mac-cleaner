...
// A cloud placeholder (iCloud, Dropbox online-only) reports its full logical
// size with no allocated blocks. Opening one makes macOS download it, and the
// open() blocks until that finishes — or forever when the provider stalls,
// which wedges a scan worker and with it the whole scan.
const isLocal = (st) => st.size === 0 || st.blocks * 512 >= st.size / 2;
...
parentPort.on('message', (task) => {
  ...
  try {
    ...
    else if (task.type === 'full-hash') fullHashTask(task);
  } catch (err) {
...
function sizeTask({ taskId, itemId, target, reportBinaries, exclude }) {
  let bytes = 0, files = 0, denied = 0;
  let lastPost = Date.now();
  // subtrees listed as their own items — counted there, skipped here
...
function walkTask(task) {
  ...
  const { maxDepth, fanoutDepth, claimed, rules, prefixRules, largeFileMin, binaryExts, binaryMin, installerExts, installerMin, downloadsDir, home, dupMin, packageSkips, opaqueBundles } = config;
  const claimedSet = new Set(claimed);
  // Photos/Music/Final Cut libraries are opaque bundles: files inside them are
  // not user documents, and offering one for deletion corrupts the library.
  // The media walk already guards this; the home walk must too, because a
  // library living in Documents or Desktop is reached from here.
  const isPackage = (name) => { const l = name.toLowerCase(); return (packageSkips || []).some(x => l.endsWith(x)); };
  // Every other macOS package (.app, .logicx, .sparsebundle, .pages …). Same
  // rule — never descend — but these get a real deletable row instead of a
  // display-only one, so they are reported under a different message.
  // A name that IS the suffix is a dot-directory, not a package: Ruby's
  // ~/.bundle is a config folder, not a plugin bundle.
  const isBundle = (name) => {
    const l = name.toLowerCase();
    return (opaqueBundles || []).some(x => l.length > x.length && l.endsWith(x));
  };
  ...
  // One directory holding hundreds of thousands of entries takes longer than
  // the parent's task timeout to iterate, and the per-directory beat below
  // fires only when a NEW directory is opened. Beat inside the entry loop too,
  // or a healthy walk of one enormous folder is killed as if it were wedged.
  let entriesSinceBeat = 0;
  ...
  while (stack.length) {
    ...
    for (const ent of ents) {
      if (isCancelled()) break;
      if (++entriesSinceBeat >= 2048) {
        entriesSinceBeat = 0;
        const t = Date.now();
        if (t - lastStatus > 300) { lastStatus = t; post({ type: 'walk-status', taskId, dir }); }
      }
      const p = dir + '/' + ent.name;
      ...
      if (ent.isDirectory()) {
        ...
        if (isBundle(ent.name)) {
          post({ type: 'found-bundle', taskId, path: p, inDownloads: dir === downloadsDir || dir.startsWith(downloadsDir + '/') });
          continue;
        }
      ...
      if (dupMin && st.size >= dupMin && isLocal(st)) {
...
function mediaWalkTask(task) {
  ...
  const { largeFileMin, dupMin, packageSkips, opaqueBundles } = config;
  let denied = 0;
  let lastStatus = 0;
  let dupBatch = [];
  const flushDups = () => { if (dupBatch.length) { post({ type: 'dup-candidates', files: dupBatch }); dupBatch = []; } };
  ...
  while (stack.length) {
    ...
    for (const ent of ents) {
      if (isCancelled()) break;
      // media walks never fan out, so ~/Pictures with a huge loose-RAW folder
      // is one single-threaded task — exactly the shape the watchdog killed
      if (++entriesSinceBeat >= 2048) {
        entriesSinceBeat = 0;
        const t = Date.now();
        if (t - lastStatus > 300) { lastStatus = t; post({ type: 'walk-status', taskId, dir }); }
      ...
      if (ent.isDirectory()) {
        ...
        if (isBundle(ent.name)) {
          post({ type: 'found-bundle', taskId, path: p, media: true });
          continue;
        }
...
// ---------------------------------------------------------------- hash ----
// Duplicate confirmation: same byte size AND same sampled digest. Three
// samples — head 256 KB, middle 64 KB, tail 64 KB — plus the exact length.
// The middle read closes the one realistic collision the two-point sample
// had: container formats (video, disk images, VM disks) that share identical
// headers and trailers around different payloads. Still O(384 KB) per file,
// so a 4 GB video costs the same as a photo.
...
function hashTask({ taskId, files }) {
  for (const f of files) {
    ...
    let fd;
    try {
      // re-check before opening: the file may have been evicted to the cloud
      // between the walk and now
      const pre = fs.lstatSync(f.path);
      if (!isLocal(pre)) continue;
      ...
      hash.update(head);
      if (st.size > head.length + 128 * 1024) {
        const mid = Buffer.alloc(64 * 1024);
        fs.readSync(fd, mid, 0, mid.length, Math.floor(st.size / 2));
        hash.update(mid);
      }
...
// ----------------------------------------------------------- full hash ----
// The sampled digest above is a FILTER, not a verdict: two files can agree on
// head, middle and tail and differ anywhere in between — cloned VM disks,
// preallocated database files and .sparsebundle bands all do. Before a
// duplicate is actually deleted its entire content is read once and hashed,
// so "identical" is a fact rather than a strong guess. Runs on a dedicated
// short-lived worker, never on the server's event loop.

function fullHashTask({ taskId, paths }) {
  ...
  for (const p of paths) {
    ...
    try {
      const st = fs.lstatSync(p);
      if (!st.isFile() || st.isSymbolicLink()) { post({ type: 'full-hash-result', taskId, path: p, hash: null }); continue; }
      const hash = crypto.createHash('sha256');
      fd = fs.openSync(p, 'r');
      let n;
      while ((n = fs.readSync(fd, buf, 0, buf.length, null)) > 0) {
        hash.update(n === buf.length ? buf : buf.subarray(0, n));
        if (isCancelled()) { fs.closeSync(fd); return; }
      }
      fs.closeSync(fd);
      fd = undefined;
      post({ type: 'full-hash-result', taskId, path: p, size: st.size, mtime: st.mtimeMs, hash: hash.digest('hex') });
    } catch {
      if (fd !== undefined) { try { fs.closeSync(fd); } catch {} }
      // unreadable is not "identical" — a null hash can never match anything
      post({ type: 'full-hash-result', taskId, path: p, hash: null });
    }
  }
}
