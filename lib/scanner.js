...
export class Scanner extends EventEmitter {
  ...
  registerDuplicates() {
    let cluster = 0;
    const clusters = [...this.hashResults.values()].filter(c => c.length >= 2)
      .sort((a, b) => b[0].size * (b.length - 1) - a[0].size * (a.length - 1));
    for (const copies of clusters.slice(0, 120)) {
      cluster++;
      ...
      copies.forEach((c, idx) => {
        ...
        if (item) {
          item.project = label;        // cluster key + header label in the UI
  ...
  registerItem(spec, { allowOverlap = false, claim = true } = {}) {
    ...
    if (!allowOverlap) {
      ...
      carve = this.claimedChildNames(real);
    ...
    const item = {
      ...
      kind: spec.kind || 'dir',
      paths: spec.paths,
      deleteMode: carve ? 'contents' : (spec.deleteMode || 'self'),
      contentsExclude: carve ? [...new Set([...(spec.contentsExclude || []), ...carve])] : spec.contentsExclude,
    ...
    if (spec.sized) {
      ...
      this.submit({
        ...
        exclude: carve ? carve.map(n => path.join(real, n)) : undefined,
        maxDepth: spec.sizeMaxDepth,
      });
    }
    if (carve) {
      item.badges = [...item.badges, `${carve.length} sub-folder${carve.length === 1 ? '' : 's'} listed separately`];
    }
    this.emit('item', item);
    return item;
  }
  ...
  // Direct child names of p that contain a claim, so a carve-out can exclude
  // whole subtrees by name (the delete path only understands top-level names).
  claimedChildNames(p) {
    const under = this.claimsUnder(p);
    if (!under.length) return null;
    const names = new Set();
    for (const c of under) names.add(c.slice(p.length + 1).split('/')[0]);
    return [...names];
  }
  ...
  sizeExternally(item) {
    const session = this.session;
    this.pendingAsync++;
    execFile('/usr/bin/du', ['-skx', item.path], { timeout: 20000, maxBuffer: 1024 * 1024 }, (err, stdout) => {
      ...
      if (!isNaN(kb)) {
        ...
        item.badges = [...item.badges, err && err.killed ? 'too slow to measure' : 'size not available'];
      }
      item.status = 'done';
      this.emit('item', item);
      this.maybeFinish();
  ...
  registerUpdaterSweep() {
    ...
    for (const ent of ents) {
      ...
      try { st = fs.lstatSync(p); } catch { continue; }
      if (!st.isDirectory()) continue;
      const overlapped = this.overlapsClaim(p);
      const item = this.registerItem({
        group: sw.group,
