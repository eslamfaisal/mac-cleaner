...
const TASK_TIMEOUT_MS = 45 * 1000;   // a directory this slow is unreadable in practice
...
// Has an IDE version folder been used lately? JetBrains and Android Studio
// rewrite <dir>/options on every exit, so it is the most direct signal
// available without asking the launcher. Falls back to the folder itself.
const IDE_RECENT_MS = 90 * 24 * 3600 * 1000;
function recentlyUsed(dir) {
  for (const probe of [path.join(dir, 'options'), dir]) {
    try {
      if (Date.now() - fs.lstatSync(probe).mtimeMs < IDE_RECENT_MS) return true;
    } catch {}
  }
  return false;
}

// The newest of a directory's own mtime and its immediate children's. One
// extra readdir, and only for candidates that already looked abandoned.
...
// ------------------------------------------------- duplicate keeper -------
// Which copy of a duplicate set should survive. Ranking on mtime alone got
// this backwards by construction: a re-downloaded file is always the newest,
// so ~/Downloads/2024-lease.pdf outranked ~/Documents/Contracts/2024-lease.pdf
// and the filed original was the one offered for deletion.
//
// mtime stays as the last tiebreak rather than being inverted — cp -p, rsync
// and Finder's Duplicate all preserve it, so "oldest" is not the original
// either.
...
// Only markers a copy operation actually produces. Deliberately not /-\d+$/,
// which would call "photo-2024" and "report-2" copies.
const COPY_MARKER = /(?: copy(?: \d+)?| \(\d+\))$/i;

function keeperRank(f) {
  const inTransient = TRANSIENT_DIRS.some(d => f.path === d || f.path.startsWith(d + '/'))
    || f.path.includes('/tmp/');
  const stem = path.basename(f.path).replace(/\.[^.]+$/, '');
  return {
    transient: inTransient ? 1 : 0,          // lower is better
    copyName: COPY_MARKER.test(stem) ? 1 : 0,
...
function compareKeeper(a, b) {
  ...
  if (ra.transient !== rb.transient) return ra.transient - rb.transient;
  if (ra.copyName !== rb.copyName) return ra.copyName - rb.copyName;
  if (ra.depth !== rb.depth) return ra.depth - rb.depth;   // shallower wins
  return rb.mtime - ra.mtime;                              // then newest
}
...
export class Scanner extends EventEmitter {
  constructor() {
    ...
    this.walkDenied = 0;
    this.truncated = [];     // roots a killed walk left unscanned
    this.enumeratedParents = new Set();  // folders whose children the catalogue lists explicitly
  ...
  state() {
    return {
      status: this.status, tasksDone: this.tasksDone, tasksTotal: this.tasksTotal,
      session: this.session, threads: THREADS,
      // both were accumulated and never read by anything, so a short scan —
      // whether from a killed walk or a folder macOS refused — looked
      // identical to a complete one
      truncated: this.truncated, walkDenied: this.walkDenied,
    };
  }
  ...
  startScan() {
    ...
    this.walkDenied = 0;
    this.truncated = [];
    this.enumeratedParents = new Set();
    this.duQueue = [];
    this.duRunning = 0;
    this.pendingAsync = 0;
    this.dupCandidates = [];
    ...
    for (let i = 0; i < THREADS; i++) this.spawnWorker(this.session);
    ...
    this.fetchSimDevices((simDevices, simctlOk) => {
      ...
      this.walkConfig = {
        ...
        opaqueBundles: WALK.opaqueBundles,
      };
  ...
  // A worker can be lost to a syscall that never returns: opening a file the
  // cloud has to download first, a stalled File Provider, an external disk
  // that stops answering. Nothing in the pool can interrupt that, so a single
  // bad path used to leave the scan at 99% forever. Give up on the task, kill
  // that thread, replace it, and carry on.
  startTaskWatchdog() {
    const session = this.session;
    clearInterval(this.watchdog);
    this.watchdog = setInterval(() => {
      if (this.session !== session || this.status !== 'scanning') { clearInterval(this.watchdog); return; }
      const now = Date.now();
      for (const [taskId, r] of [...this.running]) {
        // Against the LAST HEARTBEAT, not the dispatch time. Every task type
        // already reports progress as it works (walk-status every ~300ms,
        // size-progress, each found-*), and the watchdog ignored all of it —
        // so a walk that was merely long, not stuck, was killed at 45s. Its
        // remaining work lives in a worker-local stack and dies with the
        // thread, walk-done never arrives, and because tasksDone was bumped
        // anyway the bar still read 100% and the scan reported "done".
        // A genuinely wedged syscall emits no heartbeat and is still killed.
        if (now - (r.lastBeat || r.startedAt) < TASK_TIMEOUT_MS) continue;
        const item = r.task.itemId ? this.items.get(r.task.itemId) : null;
        if (item && item.status === 'scanning') {
          item.status = 'error';
          item.error = 'took too long to read (skipped)';
          this.emit('item', item);
        }
        // A walk killed mid-flight leaves part of the disk unscanned. Say so
        // rather than letting the totals quietly under-report.
        if (r.task.type === 'walk' || r.task.type === 'media-walk') {
          this.truncated.push(display(r.task.root || r.task.dir || '(unknown)'));
        }
        this.running.delete(taskId);
        this.tasksDone++;
        const dead = r.worker;
        this.workers = this.workers.filter(x => x !== dead);
        this.idle = this.idle.filter(x => x !== dead);
        dead.terminate();
        this.spawnWorker(session);
      }
      this.emit('progress');
      this.dispatch();
      this.maybeFinish();
  ...
  // Full-content digests for a handful of paths, on a dedicated short-lived
  // worker. Deliberately NOT the scan pool: this runs at delete time, and
  // borrowing a pool slot would both stall a running scan and corrupt its
  // task accounting. Resolves to path -> sha256 hex, or null where the file
  // could not be read (which can never match anything).
  fullHashes(paths) {
    ...
    return new Promise((resolve) => {
      ...
      try {
        w = new Worker(WORKER_PATH, { workerData: { cancelSAB: new SharedArrayBuffer(4) } });
      } catch { return finish(); }
      w.on('message', (m) => {
        if (m.type === 'full-hash-result') out.set(m.path, m.hash);
        else if (m.type === 'task-complete') finish();
      });
      w.on('error', finish);
      w.on('exit', finish);
      w.postMessage({ type: 'full-hash', taskId: 0, paths });
    });
  }
    ...
    for (const [key, copies] of clusters.slice(0, 200)) {
      ...
      copies.forEach((c, idx) => {
        ...
        const item = this.registerItem({
          ...
          kind: 'file',
          safety: 'caution',
          why: `Same size and matching sample of the file's start, middle and end as ${copies.length - 1} other file${copies.length > 2 ? 's' : ''}. The whole content is compared again before anything is deleted. Keep at least one copy.`,
          regen: 'The other copies remain untouched.',
          bytes: c.size, files: 1, mtime: c.mtime, sized: true,
          badges: keep ? ['suggested keep — filed location'] : ['duplicate copy'],
  ...
  dispatch() {
    while (this.idle.length && this.queue.length) {
      ...
      this.running.set(task.taskId, { worker: w, task, startedAt: Date.now() });
      w.postMessage(task);
    }
  }
  ...
  onWorkerMessage(w, m) {
    ...
    // any message at all is proof the task is alive (see startTaskWatchdog)
    if (m.taskId !== undefined) {
      const r = this.running.get(m.taskId);
      if (r) r.lastBeat = Date.now();
    }
    switch (m.type) {
      case 'task-complete': {
        const r = this.running.get(m.taskId);
        // Only count a task the watchdog has not already counted. A message
        // still in flight when terminate() ran used to bump tasksDone a second
        // time and push the progress bar past 100%.
        if (!r) break;
        this.running.delete(m.taskId);
        this.idle.push(r.worker);
        this.tasksDone++;
        this.emit('progress');
        this.dispatch();
        this.maybeFinish();
      ...
      case 'size-done': {
        ...
        if (m.type === 'size-done') {
          item.status = this.finalStatus(item, m);
          // A row registered only so its interior could be claimed (every
          // macOS package the walker refuses to descend into) earns its place
          // in the list only if it is actually big. 'missing' is the status the
          // UI filters out everywhere; the claim stays, so nothing inside the
          // package can be re-registered by a later task.
          if (item.hideBelow && item.bytes < item.hideBelow) item.status = 'missing';
        }
        this.emit('item', item);
        break;
      }
      ...
      case 'found-bundle': if (this.status === 'scanning') this.onFoundBundle(m); break;
  ...
  registerItem(spec, { allowOverlap = false, claim = true } = {}) {
    let real;
    // realpath on a File Provider path can block the event loop indefinitely;
    // these targets are display-only, so the literal path is enough
    if (spec.noStat) real = spec.path;
    else try { real = fs.realpathSync(spec.path); } catch { return null; }
    ...
    const item = {
      ...
      denied: 0,
      mtime: spec.mtime,
      // What the keep list is keyed on. Defaults to the path, but two rows can
      // legitimately share a path — registerAvd emits both "user data (wipe)"
      // and "(entire emulator)" for the same .avd — and pinning the small one
      // then hid the 8 GB one from every future scan.
      keepKey: spec.keepKey || spec.path,
      // when set, the row is dropped from the UI once sizing shows it is
      // smaller than this (see the size-done handler)
      hideBelow: spec.hideBelow || 0,
    };
  ...
  runDu(item, session, timeout, onFail) {
    execFile('/usr/bin/du', ['-skx', item.path], { timeout, maxBuffer: 1024 * 1024 }, (err, stdout, stderr) => {
      // registered without a stat, so "it isn't there" only shows up here
      // (a Mac before macOS 12.3 has no CloudStorage at all)
      if (/No such file/i.test(String(stderr || ''))) {
  ...
  registerStaticTargets() {
    for (const t of buildStaticTargets()) {
      // noStat targets live behind a daemon that can hang a synchronous call
      // (see categories.js) — they are registered blind and sized out-of-process
      if (t.noStat) { this.registerItem(t); continue; }
      let st;
      try { st = fs.lstatSync(t.path); } catch { continue; }
      if (t.kind === 'children' && st.isDirectory()) {
        // A childFilter is the catalogue saying "inside this folder, only
        // these children are offered". The walker must respect that instead
        // of re-adding the rest as generic package rows.
        if (t.childFilter) this.enumeratedParents.add(t.path);
  ...
  onArtifact(m) {
    // no realpath here: registerItem resolves the path and applies the same
    // claimedAncestor test, so doing it first was a second realpath on every
    // walker result for an identical outcome
  ...
  // Every other macOS package: .app, .logicx, .sparsebundle, .pages, .pvm …
  // The walker stops at the boundary, so the interior can never be offered as
  // a row or clustered as a duplicate. One row stands for the whole thing —
  // without it a 40 GB Parallels VM would simply vanish from Biggest Files.
  onFoundBundle(m) {
    // Lightroom's Catalog.lrcat-data and Mobile Downloads.lrdata are packages,
    // so the walker correctly refuses to descend — but the catalogue has
    // already decided which children of ~/Pictures/Lightroom are offered, and
    // adding the rest back as generic package rows would undo that. Claim the
    // path so nothing inside it is reachable, and offer no row.
    if (this.enumeratedParents.has(path.dirname(m.path))) {
      try { this.claims.add(fs.realpathSync(m.path)); } catch {}
      return;
    }
    // no realpath here: registerItem resolves the path and applies the same
    // claimedAncestor test, so doing it first was a second realpath on every
    ...
    const isApp = ext === '.app';
    // Downloads is where a leftover installed app really is re-downloadable.
    // Anywhere else a bundle is a document or a VM — the user's own work.
    const safety = m.inDownloads ? 'caution' : 'risky';
    const item = this.registerItem({
      group: 'large',
      name,
      path: m.path,
      safety,
      why: isApp
        ? 'An application bundle. macOS shows it as one file — it only works whole, so this row deletes all of it or none of it.'
        : 'A macOS package: one file to the Finder, a folder underneath. Deleting part of it corrupts the rest, so it is offered only as a whole.',
      regen: m.inDownloads
        ? 'Re-download if ever needed again.'
        : 'Gone for good unless backed up elsewhere.',
      badges: [isApp ? 'app bundle' : 'package'],
      // Below the Biggest Files threshold this is just a document the user
      // owns — claimed so nothing inside it is offered, but not shown.
      hideBelow: WALK.largeFileMin,
    });
    if (item && m.media) item.badges = [...item.badges, 'media'];
  }
  ...
  registerPersonal() {
    ...
    try {
      ...
      if (old.length) {
        this.registerItem({
          group: 'personal',
          name: `Downloads unchanged for 90+ days (${old.length})`,
          kind: 'files', paths: old, path: downloads,
          normalGroup: 'n-downloads',
          safety: 'caution',
          // mtime, not access time: reading a file never updates mtime, so
          // "not opened" was a claim the filter cannot make. A manual opened
          // every week is listed here and was told it had been untouched.
          why: 'Files in Downloads you have not changed in three months — usually already installed, read, or forgotten. Opening a file does not count as a change, so check the list.',
  ...
  registerApplications() {
    ...
    // Vendor installers nest their bundles one level down —
    // /Applications/Bitdefender/*.app, ~/Applications/Chrome Apps.localized/*.app.
    // A flat readdir never saw those, so the leftover matcher had no token for
    // them and labelled the running app's own data "left behind after an
    // uninstall". Collected separately and fed ONLY to that matcher: pushing
    // them into apps[] would let classifyApplications offer a vendor's helper
    // app as an unused app in its own right.
    this.extraAppNames = [];
    for (const root of roots) {
      ...
      for (const e of ents) {
        ...
        if (e.name.endsWith('.app')) { apps.push(path.join(root, e.name)); continue; }
        if (e.name === 'Utilities' || e.name.startsWith('.')) continue;
        try {
          for (const c of fs.readdirSync(path.join(root, e.name), { withFileTypes: true })) {
            if (c.isDirectory() && c.name.endsWith('.app')) {
              this.extraAppNames.push(path.basename(c.name, '.app'));
            }
          }
        } catch {}
      }
  ...
  registerStaleIdeVersions() {
    const VENDOR_ROOTS = [
      { dir: path.join(HOME, 'Library/Application Support/JetBrains'), label: 'settings & plugins', safety: 'caution',
        regen: 'Gone for good. If that version is still installed it restarts at factory defaults — plugins, keymaps and code styles included.' },
      { dir: path.join(HOME, 'Library/Application Support/Google'), label: 'settings & plugins', safety: 'caution',
        regen: 'Gone for good. If that version is still installed it restarts at factory defaults — plugins, keymaps and code styles included.' },
      // LocalHistory is this folder's one irreplaceable thing: a per-version
      // record of local edits with no copy anywhere else. Carved out of the
      // delete so "Select all safe" — which takes every safe row with no
      // per-item decision — cannot take it.
      { dir: path.join(HOME, 'Library/Caches/JetBrains'), label: 'caches & indexes', safety: 'safe',
        contentsExclude: ['LocalHistory'],
        regen: 'Rebuilt on next launch if that version is still installed — a full reindex. Local history for that version is kept.' },
    ...
    for (const root of VENDOR_ROOTS) {
      ...
      for (const list of byProduct.values()) {
        ...
        for (const old of list.slice(0, -1)) {
          const dir = path.join(root.dir, old.name);
          // Sorting directory NAMES by version proves nothing about what is
          // installed. Running 2024.3 alongside 2025.2 is routine when a
          // project pins an older platform, and this told the user "nothing
          // reads it any more" about the IDE they had open. JetBrains rewrites
          // <dir>/options on every exit, so its mtime is a direct, cheap proxy
          // for "still launched". Toolbox is not usable here: it stores build
          // numbers (243.24978.46), never marketing versions (2024.3).
          if (recentlyUsed(dir)) continue;
          this.registerItem({
            group: 'applications',
            normalGroup: 'n-dev',
            ...
            path: dir,
            safety: root.safety,
            contentsExclude: root.contentsExclude,
            deleteMode: root.contentsExclude ? 'contents' : undefined,
            why: `Belongs to ${ideProductName(old.product)} ${old.version}. Version ${newest.version} is also present and keeps its own separate settings. Nothing has written here in over 90 days; if ${old.version} is still installed and you launch it, this is the folder it uses.`,
            regen: root.regen,
  ...
  registerAppLeftovers(infos) {
    ...
    // system apps, plus bundles nested one level inside /Applications that the
    // flat scan in registerApplications cannot see (see extraAppNames there)
    for (const name of [...(this.systemAppNames || []), ...(this.extraAppNames || [])]) {
    ...
    const registerLeftover = (p, name, label, why) => {
      ...
      try {
        ...
        // A dir written in the last 60 days belongs to something still
        // installed — not a leftover. The top-level mtime alone is not that
        // signal: apps write to a SUBFOLDER, leaving the parent's own mtime
        // frozen at install time. Measured on a real machine, 13 of 185 dirs
        // in ~/Library/Application Support had a stale parent and a child
        // touched within the week, so live data was being offered as
        // abandoned. Take the newest of the folder and its direct children.
        if (Date.now() - newestMtime(p) < STALE) return;
      } catch { return; }
