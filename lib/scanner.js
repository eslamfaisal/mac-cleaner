...
export class Scanner extends EventEmitter {
  ...
  // Info.plist inside a backup is a binary plist — plutil is the only reliable
  // reader. Async, with the same pendingAsync accounting as the other probes.
  labelIosBackup(item) {
    const session = this.session;
    const plist = path.join(item.path, 'Info.plist');
    if (!fs.existsSync(plist)) return;
    this.pendingAsync++;
    execFile('/usr/bin/plutil', ['-convert', 'json', '-o', '-', plist],
      { timeout: 10000, maxBuffer: 8 * 1024 * 1024 }, (err, stdout) => {
        if (this.session !== session) return;
        this.pendingAsync--;
        if (!err) {
          try {
            const info = JSON.parse(stdout);
            const device = info['Device Name'] || info['Display Name'];
            const product = info['Product Name'] || info['Product Type'];
            const last = info['Last Backup Date'] ? new Date(info['Last Backup Date']) : null;
            if (device) {
              item.name = product && !device.includes(product) ? `${device} (${product})` : device;
              if (last && !isNaN(last)) {
    ...
    execFile('/sbin/mount', [], { timeout: 8000 }, (err, stdout) => {
      ...
      if (!err) {
        ...
        for (const line of stdout.split('\n')) {
          // "/dev/disk4s1 on /Volumes/Backup (apfs, local, ...)"
          const m = line.match(/^\/dev\/disk\S+ on (\/Volumes\/[^(]+?) \(/);
          if (!m) continue;
          const vol = m[1];
          if (line.includes('read-only')) continue;
  ...
  onFoundPackage(m) {
    ...
    this.registerItem({
      ...
      regen: 'Cannot be regenerated — this is your actual library.',
      displayOnly: true,
  ...
  registerAppLeftovers(infos) {
    ...
    const registerLeftover = (p, name, label, why) => {
      ...
      const overlapped = this.overlapsClaim(real);
      const item = this.registerItem({
        group: 'leftovers',
